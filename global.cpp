...
int Global::getWindowsVersionInfo()
{
    OSVERSIONINFOEX osInfo;
    osInfo.dwOSVersionInfoSize = sizeof(OSVERSIONINFOEX);
    ::GetVersionEx((LPOSVERSIONINFO)&osInfo);
    int x86 = isSys32Bit() ? 0 : 1;

    //windows 10 需要特殊处理
    QString strKernel32Version = Tool::getFileVersion("os.dll");
    int nKernelVersion = strKernel32Version.left(strKernel32Version.indexOf(".")).toInt();
    if(10 == nKernelVersion)
    {
        osInfo.dwMajorVersion = nKernelVersion;
        osInfo.dwMinorVersion = 0;
    }

    return (osInfo.dwMajorVersion * 100) + (osInfo.dwMinorVersion * 10) + x86;
}
